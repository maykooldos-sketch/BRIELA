    ...
    return (
        <nav className="navbar">
            <div className="container nav-content">
                {/* LADO IZQUIERDO: Menú Móvil / Enlaces Desktop Izquierda */}
                ...
                {/* CENTRO: Logo siempre protagonista */}
                <Link to="/" className="logo" onClick={() => setIsMenuOpen(false)}>
                    BRIELA
                </Link>

                {/* LADO DERECHO: Buscador y Cuenta */}
                        ...
                        {user ? (
                            <>
                                ...
                                <div className="user-dropdown">
                                    <Link to="/dashboard" className="user-name">Cuenta</Link>
                                    <div className="dropdown-content">
                                        <Link to="/dashboard">Mis Pedidos</Link>
                                        {user.role === 'admin' && <Link to="/admin">Panel Control</Link>}
                                        <button onClick={handleLogout} className="logout-link">Salir</button>
                                    </div>
                                </div>
                            </>
                        ) : (
                            <Link to="/login" className="login-btn">Acceder</Link>
                        )}
                    </div>

                    {/* Icono carrito para móvil */}
                ...
                <div className="drawer-links">
                    ...
                    {user ? (
                        <>
                            ...
                            <Link to="/cart" onClick={() => setIsMenuOpen(false)}>Mi Carrito</Link>
                            <Link to="/dashboard" onClick={() => setIsMenuOpen(false)}>Mis Pedidos</Link>
                            {user.role === 'admin' && <Link to="/admin" onClick={() => setIsMenuOpen(false)}>Control Panel</Link>}
                            <button onClick={handleLogout} className="drawer-logout">Cerrar Sesión</button>
                        </>
                    ) : (
            ...
            {/* WhatsApp - fuera de la nav para que flote siempre */}
